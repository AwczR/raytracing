...
class Triangle():
    def __init__(self,point0,point1,point2,type):
        self.discription=np.array([point0,point1,point2])
        self.type=type
...
def intersect_sphere(O, D, sphere):#检测球体与射线起点距离
    A = np.dot(D, D)
...
def trace_ray(O, D, balls, lights, depth,ground):
    ...
    if True:#balls
        ...
        if closest_sphere.texture=='mirror':
            ...
            return np.clip(color,0,1)
        #if closest_sphere.texture=='glass' :
...
def run():
    ...
    balls = [
        ...
        Sphere(np.array([ 6, 0, 30]), 30.0, np.array([1, 1, 1]), 'simple'),
